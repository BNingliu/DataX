package cn.alibaba.datax.plugin.writer.greenplumwriter;
...
public class GpdbWriter extends Writer {
	private static final Logger LOG = LoggerFactory.getLogger(GpdbWriter.class);
		...
		public void init() {
			...
			if (null != writeMode) {
				LOG.info(String.format(
						" 因为Greenplum Database不支持配置参数项 writeMode: %s, " +
								"Greenplum Database仅使用insert sql 插入数据. 所以写入模式(writeMode)实际不生效.",
						writeMode));
