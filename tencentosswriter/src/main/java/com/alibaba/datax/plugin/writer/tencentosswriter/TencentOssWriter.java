package com.alibaba.datax.plugin.writer.tencentosswriter;
...
import com.alibaba.datax.plugin.writer.tencentosswriter.util.CosUtil;
...
public class TencentOssWriter extends Writer {
    ...
    public static class Job extends Writer.Job {
        ...
        public void init() {
            ...
            this.cosClient = CosUtil.initCosClient(this.writerSliceConfig);
        }
        ...
        public void prepare() {
            ...
            try {
                ...
                if (!this.cosClient.doesBucketExist(bucket)) {
                    // this.cosClient.createBucket(bucket);
                ...
                LOG.info(String.format("access control details [%s].",
                        this.cosClient.getBucketAcl(bucket).toString()));
                ...
                if ("truncate".equals(writeMode)) {
                    ...
                    while (true) {
                        ...
                        listing = this.cosClient.listObjects(bucket, object);
                        List<COSObjectSummary> objectSummarys = listing.getObjectSummaries();
                        for (COSObjectSummary objectSummary : objectSummarys) {
                            LOG.info(String.format("delete oss object [%s].", objectSummary.getKey()));
                            this.cosClient.deleteObject(bucket, objectSummary.getKey());
                        }
                    ...
                    LOG.info(String
                            .format("由于您配置了writeMode nonConflict, 开始检查Bucket [%s] 下面以 [%s] 命名开头的Object",
                                    bucket, object));
                    ObjectListing listing = this.cosClient.listObjects(bucket,
                            object);
                    if (0 < listing.getObjectSummaries().size()) {
                        StringBuilder objectKeys = new StringBuilder();
                        objectKeys.append("[ ");
                        for (COSObjectSummary ossObjectSummary : listing.getObjectSummaries()) {
        ...
        public List<Configuration> split(int mandatoryNumber) {
            ...
            try {
                List<COSObjectSummary> ossObjectlisting = this.cosClient.listObjects(bucket).getObjectSummaries();
                for (COSObjectSummary objectSummary : ossObjectlisting) {
