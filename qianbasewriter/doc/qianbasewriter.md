...
QianBaseWriter插件实现了写入数据到 QianBase Database 主库目的表的功能。在底层实现上，QianBaseWriter通过JDBC连接远程 QBDB 数据库，并执行相应的 Copy FROM 语句将数据写入 QBDB。

QianBaseWriter面向ETL开发工程师，他们使用QianBaseWriter从数仓导入数据到QBDB。同时 QianBaseWriter亦可以作为数据迁移工具为DBA等用户提供服务。


## 2 实现原理

QianBaseWriter通过 DataX 框架获取 Reader 生成的协议数据，根据你配置生成相应的SQL插入语句
...
<br />
    ...
    注意：
    1. QianBaseWriter和MysqlWriter不同，不支持配置writeMode参数。
...
* 这里使用一份从内存产生到 QianBaseWriter导入的数据。
...
目前 QianBaseWriter支持大部分 PostgreSQL类型，但也存在部分没有支持的情况，请注意检查你的类型。

下面列出 QianBaseWriter针对 PostgreSQL类型转换列表:
...
**Q: QianBaseWriter 执行 postSql 语句报错，那么数据导入到目标数据库了吗?**
...
**Q: 为什么不用postgresqlwriter插件，而要开发一个新的插件QianBaseWriter？**

A: 对于QBDB 的Append optimized table 和 HAWQ，单条记录插入的效率非常低，而QianBaseWriter插件使用copy from语句而不是insert into语句，性能有巨大提升。

**Q: 为什么会有单个元组 4MB 大小的限制？**

A: 因为 QBDB 服务端 对 COPY FROM 的 CSV 格式做了这样的限制，如果单个元组大于4 MB，只能使用 insert。对于这种情况，推荐使用 postgresqlwriter。
